...
// Migrado para novo projeto em 2026-05-20
const supabaseUrl = 'https://uchxrfbnaofqpqzcxnyt.supabase.co'
...
export async function verifyUsersAndCaedTable() {
  ...
  if (firstCheck.error) {
    ...
    if (createTry.error) {
      console.error("[supabase] tabela users_and_caed nao disponivel:", createTry.error.message)
      return false
    }
    const secondCheck = await supabase.from("users_and_caed").select("id").limit(1)
    if (secondCheck.error) {
      console.error("[supabase] falha ao validar tabela users_and_caed:", secondCheck.error.message)
      return false
    }
  }
